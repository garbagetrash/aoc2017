...
class Virus():
    ...
    def turn(self, world):
        ...
        if state == 1:
            self.turn_right()
            world.flag(self.row, self.col)
        elif state == 0:
            self.turn_left()
            world.weaken(self.row, self.col)
        elif state == 2:
            ...
            print(world.dict[self.row, self.col])
...
if __name__ == '__main__':
    assert name(['..#', '#..', '...'], N=100) == 26
    assert name(['..#', '#..', '...'], N=10000000) == 2511944

    with open(sys.argv[1]) as f:
        output = name([l.strip() for l in f.readlines()], N=10000000)
